...
void Player::setPipelineString(const QString & pipelineString)
{
    if (!m_pipeline) 
	{
    ...
    if (m_pipeline) 
	{
        m_pipeline->setProperty("pipelineString", pipelineString);
    }
}
...
void Player::paintEvent(QPaintEvent *evt)
{
	QGst::Ui::VideoWidget::paintEvent(evt);
}
