...
Rectangle {
    ...
    function activeUasSet() {
		rollPitchIndicator.rollAngle = Qt.binding(function() { return currentState.roll})
        rollPitchIndicator.pitchAngle = Qt.binding(function() { return  currentState.pitch})
        pitchIndicator.rollAngle = Qt.binding(function() { return currentState.roll})
        pitchIndicator.pitchAngle = Qt.binding(function() { return  currentState.pitch})
        speedIndicator.groundspeed = Qt.binding(function() { return currentState.groundspeed})
        informationIndicator.groundSpeed = Qt.binding(function() { return currentState.groundspeed})
        informationIndicator.airSpeed = Qt.binding(function() { return currentState.airspeed })
        informationIndicator.batVoltage = Qt.binding(function() { return currentState.batteryVoltage })
        informationIndicator.batCurrent = Qt.binding(function() { return currentState.batteryCurrent })
        informationIndicator.batPercent = Qt.binding(function() { return currentState.batteryRemaining })
        informationIndicator.navMode = Qt.binding(function() { return currentState.flightMode })

        compassIndicator.heading = Qt.binding(function() {
            return (currentState.yaw < 0) ? currentState.yaw + 360 : currentState.yaw ;
        })
        
		speedIndicator.airspeed = Qt.binding(function() { return currentState.airspeed } )
        altIndicator.alt = Qt.binding(function() { return currentState.altitude } )
	}
	...
	Component.onCompleted:
	{
		...
		pipelineString.text = Settings.get("pipelineString", "");
	}
	...
	Rectangle
	{
		...
		Column
		{
			...
			TextField
			{
				...
				height: 20
				z:3
				Binding { target: container; property: "pipelineString"; value: pipelineString.text }
				onEditingFinished:
				{
					Settings.set("pipelineString", pipelineString.text)				
				}
			}
		}
